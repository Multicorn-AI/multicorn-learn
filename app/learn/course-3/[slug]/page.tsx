...
import {
  getAllCourse3LessonSlugs,
  getAllCourse3Lessons,
  getCourse3Lesson,
  getCourse3LessonNavigation,
} from '@/lib/course-3'
import { COURSE_3 } from '@/lib/course-3-config'
import { extractTableOfContents } from '@/lib/learn'
import { course3Components } from '@/lib/mdx-course2-components'
...
interface Course3LessonPageProps {
  readonly params: Promise<{ slug: string }>
}

export async function generateStaticParams() {
  const slugs = getAllCourse3LessonSlugs()
  return slugs.map((slug) => ({ slug }))
}

export async function generateMetadata({ params }: Course3LessonPageProps): Promise<Metadata> {
  const { slug } = await params
  const lesson = getCourse3Lesson(slug)
  ...
  return {
    title: `${lesson.meta.title} | ${COURSE_3.title} | Multicorn Learn`,
    ...
    openGraph: {
      ...
      url: `${SITE_URL}${COURSE_3.basePath}/${lesson.slug}`,
    },
    twitter: {
...
export default async function Course3LessonPage({ params }: Course3LessonPageProps) {
  if (!isCourse3Enabled()) {
    redirect('/learn')
  }
