...
import {
  getAllCourse3AwsLessonSlugs,
  getAllCourse3AwsLessons,
  getCourse3AwsLesson,
  getCourse3AwsLessonNavigation,
} from '@/lib/course-3-aws'
...
interface Course3AwsLessonPageProps {
  readonly params: Promise<{ slug: string }>
}

export async function generateStaticParams() {
  const slugs = getAllCourse3AwsLessonSlugs()
  return slugs.map((slug) => ({ slug }))
}

export async function generateMetadata({ params }: Course3AwsLessonPageProps): Promise<Metadata> {
  const { slug } = await params
  const lesson = getCourse3AwsLesson(slug)
  ...
  return {
    title: `${lesson.meta.title} | ${COURSE_3_AWS.title} | Multicorn Learn`,
    ...
    openGraph: {
      ...
      url: `${SITE_URL}${COURSE_3_AWS.basePath}/${lesson.slug}`,
    },
    twitter: {
...
export default async function Course3AwsLessonPage({ params }: Course3AwsLessonPageProps) {
  if (!isCourse3Enabled()) {
    redirect('/learn')
  }
            ...
            <article className="mx-auto max-w-3xl lg:mx-0">
              ...
              <div className="prose-multicorn">
                <MDXRemote
                  ...
                  components={course3AwsComponents}
