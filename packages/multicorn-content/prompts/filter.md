...
You are helping choose articles for a Learn section about practical AI for teams.
...
`{ "url": "<exact URL from the list>", "relevant": true | false, "reason": "<one short sentence>" }`

Include one object per article. Use the exact `url` values from the input.
