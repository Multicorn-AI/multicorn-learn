...
import {
  getRecommendation,
  TOOL_PICKER_QUESTIONS,
  TOOL_RECOMMENDATIONS,
  type BudgetAnswer,
  type BuildAnswer,
  type TerminalAnswer,
  type ToolPickerAnswers,
  type ToolSlug,
} from '@/lib/tool-picker-constants'
...
export function ToolPicker() {
  ...
  const [, setAnswers] = useState<Partial<ToolPickerAnswers>>({})
  const [transitioning, setTransitioning] = useState(false)
  const [result, setResult] = useState<{ slug: ToolSlug; reason: string } | null>(null)
  ...
  const handleSelect = useCallback(
    (rawValue: string) => {
      ...
      if (phase === 'q1') {
        const value = rawValue as TerminalAnswer
        advanceAfterTransition(() => {
          setAnswers((a) => ({ ...a, terminal: value }))
          setPhase('q2')
        })
        return
      }
      if (phase === 'q2') {
        const value = rawValue as BuildAnswer
        advanceAfterTransition(() => {
          setAnswers((a) => ({ ...a, build: value }))
          setPhase('q3')
        })
        return
      }
      const budget = rawValue as BudgetAnswer
      advanceAfterTransition(() => {
        let nextAnswers: ToolPickerAnswers | undefined
        flushSync(() => {
          setAnswers((prev) => {
            if (prev.terminal === undefined || prev.build === undefined) {
              return prev
            }
            nextAnswers = {
        ...
        if (nextAnswers !== undefined) {
          setResult(getRecommendation(nextAnswers))
          setPhase('result')
        }
      })
    },
  ...
  const handleBack = useCallback(() => {
    ...
    advanceAfterTransition(() => {
      if (phase === 'q2') {
        setAnswers((a) => (a.terminal !== undefined ? { terminal: a.terminal } : {}))
        setPhase('q1')
      } else if (phase === 'q3') {
        setAnswers((a) => {
          if (a.terminal === undefined || a.build === undefined) return {}
          return { terminal: a.terminal, build: a.build }
        })
        setPhase('q2')
      }
    })
  ...
  const currentQuestion = questionIndex >= 0 ? TOOL_PICKER_QUESTIONS[questionIndex] : undefined
  ...
  const recommendation = result ? TOOL_RECOMMENDATIONS[result.slug] : null
