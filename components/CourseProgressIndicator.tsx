...
function PillLink({
  ...
  if (state === 'active') {
    return (
      <Link
        href={href}
        className={`${base} border-primary/30 bg-primary/10 text-primary hover:border-primary/40 focus:ring-primary/20`}
        title={label}
        aria-current="step"
      >
        <span className="sm:hidden">{shortLabel}</span>
  ...
  return (
    <Link
      href={href}
      className={`${base} border-border bg-surface-tertiary text-text-tertiary hover:border-border hover:bg-surface-secondary hover:text-text-secondary focus:ring-primary/20`}
      title={label}
    >
      <span className="sm:hidden">{shortLabel}</span>
      <span className="hidden sm:inline">{label}</span>
...
export function CourseProgressIndicator({ activeCourse }: CourseProgressIndicatorProps) {
  ...
  return (
    ...
    >
      <PillLink label="Course 1" shortLabel="C1" state={c1} href={href1} />
      <Connector />
      <PillLink label="Course 2" shortLabel="C2" state={c2} href={href2} />
      <Connector />
      <PillLink label="Course 3" shortLabel="C3" state={c3} href={href3} />
    </div>
  )
}
