...
<SecurityNote>
  Before you push your code anywhere, open your project folder and check two files. First, confirm
  `.env` is listed in your `.gitignore` file. Second, run `git status` and make sure `.env` is NOT
  in the list of tracked files. If you see `.env` in either the wrong place, stop and fix it before
  the next lesson. Lesson 3 covers this in detail, but the check takes ten seconds and it matters.
</SecurityNote>
