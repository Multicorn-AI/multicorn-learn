...
3. The full error message, copied exactly.

This works better than you would expect. Most deployment errors are very common and have well-known fixes.

<SecurityNote>
  When you paste an error message into any AI tool, scan it first for anything sensitive. Error
  messages sometimes include parts of your database connection string, a token, or the path to a
  file on the host. Replace anything that looks like a secret with `REDACTED` before you send it. If
  the error itself is about a token being invalid, rotate that token on the service that issued it.
