...
<SecurityNote>
  If your key is already in your GitHub repository, assume it is compromised. Rotate it right now,
  before you do anything else. "Rotate" means: go to the service that gave you the key (OpenAI,
  Stripe, your database provider), delete the old key, generate a new one, and put the new one in
  your `.env` file. This takes about two minutes and it is the only way to protect yourself.
  Removing the key from your code without rotating it does nothing - copies of it still exist in
  your git history, on backup servers, and probably already in the hands of bots that scraped it
  within seconds of you pushing the commit.
</SecurityNote>
